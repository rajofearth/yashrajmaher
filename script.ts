...
async function main() {
	...
	const files = [
		...fs.readdirSync(BPOSTS_DIR).filter(file => file.endsWith('.md')).map(file => ({ dir: BPOSTS_DIR, file })),
		...fs.readdirSync(DEVPOSTS_DIR).filter(file => file.endsWith('.md')).map(file => ({ dir: DEVPOSTS_DIR, file })),
	];
	...
	for (const { dir, file } of files) {
		...
		const featuredImage = frontmatter.featuredImage ? String(frontmatter.featuredImage) : null;
		const author = String(frontmatter.author || "Yashraj Maher");
		const authorImage = frontmatter.authorImage ? String(frontmatter.authorImage) : null;
		const status = String(frontmatter.status || "published");
		const website = frontmatter.website ? String(frontmatter.website) : null;
		const tagsField = frontmatter.tags;
		const tags = Array.isArray(tagsField)
			? tagsField.map(tag => String(tag)).join(',')
			: (typeof tagsField === 'string' ? tagsField : null);
