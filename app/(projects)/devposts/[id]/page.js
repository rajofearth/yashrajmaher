...
export default async function DevpostPost({ params }) {
	try {
		...
		if (!fs.existsSync(DEVPOSTS_DIR)) {
			return (
				<ErrorPage title="Project directory not found" message="Project directory not found" backLink="/devposts" />
			);
		}
		...
		if (!frontmatter.title) {
			return (
				<ErrorPage title="Invalid project format" message="Project data is incomplete" slug={id} backLink="/devposts" />
			);
		}
		...
		const formattedDate = frontmatter.date ? format(new Date(frontmatter.date), "MMMM d, yyyy") : "No date specified";
